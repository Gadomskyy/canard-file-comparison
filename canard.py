...
a = input("1. Fixed Cameras \n2. Red Light Cameras\n3. Average Speed Zones\n")

if a == '1':
    x = requests.post('https://www.canard.gitd.gov.pl/canard-cms-dodatki-rest/fotoradary/pobierzPP')
    name = 'FIX'
elif a == '2':
    x = requests.post('https://www.canard.gitd.gov.pl/canard-cms-dodatki-rest/fotoradary/pobierzRL')
    name = 'RL'
elif a == '3':
    x = requests.post('https://www.canard.gitd.gov.pl/canard-cms-dodatki-rest/fotoradary/pobierzOPP')
    name = 'ASZ'
else:
    sys.exit()
...
with open(f'CANARD_{name}_{curr_date.month}-{curr_date.year}.csv', 'w', newline='') as file:
...
new_version = os.path.join(os.getcwd(), f'CANARD_{name}_{curr_date.month}-{curr_date.year}.csv')
...
#Save result file
df_highlight.to_csv(f'Result_files/CANARD_{name}_{curr_date.month}-{curr_date.year}-results.csv')
